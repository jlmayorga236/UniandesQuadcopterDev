...
class ThreadControl (threading.Thread):
    ...
    def run(self):
        ...
        global M4
        while True:
		M1 = 1.5*max(-5,min(5,0  + 0.25*(0 - Roll) + 0.1*(0 - Pitch)  + 0.25*(Roll - oRoll) + 0.25*(Pitch - oPitch) ))
		M2 = 1.5*max(-5,min(5,0  - 0.25*(0 - Roll) + 0.1*(0 - Pitch)  + 0.25*(Roll - oRoll) + 0.25*(Pitch - oPitch)))
		M3 = 1.2*max(-5,min(5,0  + 0.15*(0 - Roll) - 0.05*(0 - Pitch)   + 0.25*(Roll - oRoll) + 0.25*(Pitch - oPitch)))
		M4 = 1.2*max(-5,min(5,0  - 0.15*(0 - Roll) - 0.05*(0 - Pitch)   + 0.25*(Roll - oRoll) + 0.25*(Pitch - oPitch)))
		z = -397*ADC.read("P9_40")+165
		TH = max(85,min(99, 85 + 1*(115-z)))
		print TH	
		TH = 90
